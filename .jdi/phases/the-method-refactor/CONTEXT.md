...
- [ ] Achado #3: `ParsingEngine` vira `partial class`, os 7 padroes regex inline viram
      `[GeneratedRegex]`, zero `Regex.Replace/Match/IsMatch` estaticos restantes
      **Verify:** `test $(grep -cE "Regex\.(Replace|Match|IsMatch)\(" src/TranslateReader.Core/Business/Engines/ParsingEngine.cs) -eq 0 && grep -q "public partial class ParsingEngine" src/TranslateReader.Core/Business/Engines/ParsingEngine.cs && test $(grep -c "\[GeneratedRegex" src/TranslateReader.Core/Business/Engines/ParsingEngine.cs) -ge 7`
      **Source:** CONTEXT
