...
- [ ] **DoD 8 — Job de CI iOS existe e esta bem formado, os outros tres jobs continuam intactos, e o
      ...
      **Verify:** `W=.github/workflows/ci.yml; R=.jdi/agents/jdi-reviewer-translatereader.md; test -f "$W" && ! grep -q "$(printf '\t')" "$W" && grep -qE '^  build-ios:' "$W" && grep -qE '^ +runs-on: macos' "$W" && grep -qF 'dotnet workload install maui-ios' "$W" && grep -qF 'dotnet build src/TranslateReader/TranslateReader.csproj -c Release -f net10.0-ios' "$W" && grep -qF 'dotnet build src/TranslateReader/TranslateReader.csproj -c Release -f net10.0-android' "$W" && grep -qF 'dotnet build src/TranslateReader/TranslateReader.csproj -c Release -f net10.0-windows10.0.19041.0' "$W" && grep -qF 'dotnet test test/TranslateReader.Tests/TranslateReader.Tests.csproj -c Release' "$W" && test "$(grep -c 'actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1' "$W")" -ge 4 && test "$(grep -c 'actions/setup-dotnet@a98b56852c35b8e3190ac28c8c2271da59106c68' "$W")" -ge 4 && test "$(grep -cE '@(v[0-9]+|main|master)[[:space:]]*$' "$W")" -eq 0 && grep -qF 'dotnet build src/TranslateReader/TranslateReader.csproj -c Release -f net10.0-android' "$R" && test "$(grep -cE '^dotnet build -f net10\.0-android$' "$R")" -eq 0 && test "$(grep -c 'a missing workload is reported as WARN, never as BLOCK' "$R")" -eq 0 && grep -qF 'Android build failure = BLOCK' "$R" && grep -qF 'iOS build is CI-only' "$R"`
